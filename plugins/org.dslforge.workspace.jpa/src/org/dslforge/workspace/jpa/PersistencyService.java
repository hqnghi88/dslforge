...
public class PersistencyService implements IPersistencyService {
	...
	private void inializeFromPersistencyUnit() {
		...
		String dburl = (String) properties.get(IPersistencyService.JAVAX_PERSISTENCE_JDBC_URL);
		String[] split = dburl.split(";");
		String dbPath = split[0].substring(IPersistencyService.JDBC_PREFIX.length(), split[0].length());
		String[] workspacePath = dbPath.split(IPersistencyService.METADATA_FOLDER);
